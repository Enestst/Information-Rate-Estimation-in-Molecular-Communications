...
MODEL_SAVE_PATH = os.path.join(BASE_DIR, "ber_multiscale_rescnn_multiobjective_large_100epoch_v2.pth")
SCALER_SAVE_PATH = os.path.join(BASE_DIR, "ber_multiscale_rescnn_multiobjective_scalers_large_100epoch_v2.pkl")
...
# =========================
class MultiObjectiveBERLoss(nn.Module):
    """
    Multi-objective loss:
    ...
    Model predicts y_log = log10(BER)
    """
    def __init__(
        self,
        log_delta=0.5,
    ...
    def forward(self, pred_log, target_log):
        # Raw BER reconstructed from log10(BER)
        pred_raw = torch.pow(10.0, pred_log).clamp(min=EPS, max=1.0)
        target_raw = torch.pow(10.0, target_log).clamp(min=EPS, max=1.0)
...
class BERMultiScaleResCNNv2(nn.Module):
    ...
    def forward(self, seq, threshold):
        ...
        return self.head(fused)  # predicts log10(BER)
...
def train_engine():
    ...
    # Multi-objective loss:
    # - keeps log-space stability
    # - also penalizes raw BER error
    # - gives extra importance to higher BER regime
    criterion = MultiObjectiveBERLoss(
        log_delta=0.5,
        raw_delta=0.01,
        alpha_log=0.7,
        beta_raw=0.3,
    ...
    for epoch in range(100):
        ...
        for b_seq, b_thr, b_y_log in train_loader:
            b_seq = b_seq.to(device, non_blocking=True)
            b_thr = b_thr.to(device, non_blocking=True)
            b_y_log = b_y_log.to(device, non_blocking=True)
        ...
        if val_rmse_log < best_val_rmse_log:
            ...
            wait = 0
            print(f"  -> New best model saved at epoch {epoch+1} with Val RMSE(log10): {val_rmse_log:.4f}")
        else:
            wait += 1
            if wait >= patience:
