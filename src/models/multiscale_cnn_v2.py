...
MODEL_SAVE_PATH = os.path.join(BASE_DIR, "ber_multiscale_rescnn_attn_v2_large.pth")
SCALER_SAVE_PATH = os.path.join(BASE_DIR, "ber_multiscale_rescnn_attn_v2_large_scalers.pkl")
...
class WeightedHuberLoss(nn.Module):
    """
    Mild weighted Huber in log10(BER) space.
...
class DownsampleResidualBlock(nn.Module):
    """
    Learnable downsampling instead of fixed AvgPool.
    """
    def __init__(self, in_ch, out_ch, dropout=0.1, se=True):
        super().__init__()
...
class ThresholdFiLM(nn.Module):
    """
    Feature-wise linear modulation from threshold embedding.
    More flexible than the earlier fixed 0.1 scaling.
    """
...
class LightSelfAttention1D(nn.Module):
    """
    Lightweight Transformer-style block on [B, C, L] features.
    Applied after downsampling so sequence length is smaller.
    """
...
class BERMultiScaleResCNNv2(nn.Module):
    ...
    Design improvements:
      - separate mean / variance stems
      - no generic extra stem mixing everything too early
      - multi-scale fusion in head from stage1/stage2/stage3/bottleneck
      - lightweight self-attention after stage3
      - stronger threshold conditioning
    """
    def __init__(
        self,
        threshold_dim=1,
        ...
        # Extra channels are injected later, not put through their own full stem.
        ...
        # Structured head: hidden summary + final scalar
    ...
    def forward(self, seq, threshold):
        ...
        var_x = seq[:, 1:2, :]    # [B,1,L]
        extra_x = seq[:, 2:5, :]  # [B,3,L] = [pos, abs_tap, snr_proxy]
...
def prepare_data(csv_path, batch_size=256, nrows=2000000, num_workers=0):
    ...
    (
        ...
        temp_taps_raw, temp_vars_feat, temp_abs_raw, temp_snr_raw, temp_thr, temp_y,
        **split_args2
    )
...
def train_engine():
    ...
    # Schedule on RMSE, not weighted loss
