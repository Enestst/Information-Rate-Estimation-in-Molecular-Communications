...
MODEL_SAVE_PATH = os.path.join(BASE_DIR, "ber_tcn_v2_fixed.pth")
SCALER_SAVE_PATH = os.path.join(BASE_DIR, "ber_tcn_v2_fixed_scalers.pkl")
...
class ChannelHistoryTCN(nn.Module):
    """
    Input sequence: [B, 3, L]
      channel 0 -> taps
      channel 1 -> variances
      channel 2 -> normalized position

    Separate threshold branch, then fusion head.
    Output: predicted log10(BER)
    """
    def __init__(self, seq_channels=3, threshold_dim=1, dropout=0.1):
        super().__init__()
        ...
        self.tcn = nn.Sequential(
            ...
            ResidualTCNBlock(128, 128, kernel_size=3, dilation=16, dropout=dropout),
        )
...
def prepare_data(csv_path, batch_size=256, nrows=500000, num_workers=0):
    ...
    # Stratified split by target distribution if possible
    ...
    # Scale each input branch separately
    ...
    t_pos = np.tile(pos, (t_taps.shape[0], 1))
    v_pos = np.tile(pos, (v_taps.shape[0], 1))
    te_pos = np.tile(pos, (te_taps.shape[0], 1))

    # [N, C, L]
    t_seq = np.stack([t_taps, t_vars, t_pos], axis=1).astype(np.float32)
    v_seq = np.stack([v_taps, v_vars, v_pos], axis=1).astype(np.float32)
    te_seq = np.stack([te_taps, te_vars, te_pos], axis=1).astype(np.float32)
...
def train_engine():
    ...
    model = ChannelHistoryTCN(seq_channels=3, threshold_dim=1, dropout=0.1).to(device)

    # Reverted closer to v1 for easier optimization
    optimizer = optim.AdamW(model.parameters(), lr=3e-4, weight_decay=1e-5)

    # Keep weighted loss, but make it milder
    criterion = WeightedHuberLoss(delta=0.5, low_ber_weight=0.05)

    # IMPORTANT: schedule using RMSE, not weighted loss
    ...
    for epoch in range(60):
        ...
        # Scheduler now follows the metric you actually care about
        ...
        # IMPORTANT: checkpoint based on RMSE, not weighted loss
    ...
    # Extra diagnostics by BER range
